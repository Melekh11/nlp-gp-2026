...
def infer_role(text: str) -> str:
    for alias, role in ROLE_ALIASES.items():
        if alias in text:
            return role
    if any(
        token in text for token in ["не знаю", "подбери", "не определился", "любая"]
    ):
        return "unknown"
    return "unknown"
...
def parse_salary(
    text: str, values: List[Any]
) -> Tuple[Optional[float], Optional[float]]:
    has_signal = (
        bool(values)
        or any(
            word in text
            for word in ["зарплат", "ожидан", "руб", "тыс", "оклад", "доход"]
        )
        or bool(re.search(r"\d+([\.,]\d+)?\s*(к|k)\b", text))
    )
    if not has_signal:
        return None, None
    numbers = []
...
def infer_skills(
    text: str, explicit: List[Any], allow_free_text: bool = False
) -> List[str]:
...
def infer_projects(text: str, explicit: List[Any]) -> List[str]:
    ...
    ) or ("бариста" in text and "проект" not in text):
...
def is_role_skill_question(text: str) -> bool:
    return (
        text.endswith("?")
        and any(word in text for word in ["навык", "скилл", "уметь", "требован"])
        and infer_role(text) != "unknown"
    )
...
def infer_project_role(text: str, value: Any = None) -> str:
    ...
    if any(
        phrase in text
        for phrase in ["никакая роль", "роли не было", "не было роли", "нет роли"]
    ):
        return "unknown"
    return "unknown"
...
def normalize_availability(text: str, value: Any = None) -> str:
    ...
    if any(
        word in raw
        for word in ["сразу", "сейчас", "now", "немедленно", "хоть завтра", "завтра"]
    ):
        return "available_now"
    if any(
        word in raw
        for word in ["три месяца", "3 месяца", "не раньше", "не готов", "позже"]
    ):
        return "available_not_soon"
    if any(
        word in raw for word in ["недел", "месяц", "скоро", "отрабаты", "следующем"]
    ):
...
def infer_education(text: str) -> Tuple[List[str], List[str]]:
    ...
    if any(
        word in text for word in ["информ", "программ", "математ", "физмат", "computer"]
    ):
...
def extract_facts(tracker: Tracker) -> Dict[str, Any]:
    ...
    years = (
        parse_years(text, entities(tracker, "experience_years"))
        if requested_slot == "experience_years" or entities(tracker, "experience_years")
        else None
    )
    ...
    skills = (
        infer_skills(
            text, entities(tracker, "skill"), allow_free_text=requested_slot == "skills"
        )
        if requested_slot == "skills"
        or latest_intent in {"provide_skills", "provide_multiple_fields"}
        else []
    )
    if skills:
    ...
    projects = (
        infer_projects(text, entities(tracker, "project_type"))
        if requested_slot == "project_types" or has_project_context(text)
        else []
    )
    if projects:
    ...
    project_role = (
        infer_project_role(text, (entities(tracker, "project_role") or [None])[0])
        if requested_slot == "project_role"
        or any(
            word in text
            for word in ["лид", "менедж", "разработ", "аналит", "управлял", "управляла"]
        )
        else "unknown"
    )
    ...
    english = (
        normalize_english(text, (entities(tracker, "english_level") or [None])[0])
        if requested_slot == "english_level" or entities(tracker, "english_level")
        else "unknown"
    )
    if english != "unknown":
        facts["english_level"] = english
    work_format = (
        normalize_work_format(text, (entities(tracker, "work_format") or [None])[0])
        if requested_slot == "work_format" or entities(tracker, "work_format")
        else "unknown"
    )
    if work_format != "unknown":
        facts["work_format"] = work_format
    availability = (
        normalize_availability(text, (entities(tracker, "availability") or [None])[0])
        if requested_slot == "availability" or entities(tracker, "availability")
        else "unknown"
    )
    ...
    salary_min, salary_max = (
        parse_salary(text, entities(tracker, "salary_amount"))
        if requested_slot == "salary_expectation_min"
        or entities(tracker, "salary_amount")
        else (None, None)
    )
...
def score_candidate(
    slots: Dict[str, Any], tie_answer: Optional[str] = None
) -> Dict[str, Any]:
    ...
    for role, weights in ROLE_SKILL_WEIGHTS.items():
        ...
        if role == "data_analyst" and (
            "analytics" in projects or project_role == "analyst"
        ):
            score += 18
        ...
        if complexity == "high":
            ...
            score -= 6
        if "technical" in education_fields and role in {
            "data_engineer",
            "data_scientist",
            "mlops_engineer",
        }:
    ...
    if (
        top["score"] >= 45
        and abs(top["score"] - second["score"]) <= 7
        and not tie_answer
    ):
...
def make_risks(slots: Dict[str, Any], top_role: str, top_score: float) -> List[str]:
    ...
    if english in {"none", "a1", "a2", "unknown"}:
        risks.append(
            "английский может ограничить работу с документацией и международной командой"
        )
...
def build_candidate_summary(
    slots: Dict[str, Any], result: Dict[str, Any]
) -> Dict[str, Any]:
    return {
...
def human_reasons(result: Dict[str, Any]) -> List[str]:
    ...
    for reason in reasons:
        ...
        cleaned.append(item)
    return cleaned[:3] or [
        "в ответах есть несколько релевантных сигналов для этой роли"
    ]
...
def human_follow_up_questions(result: Dict[str, Any]) -> List[str]:
    ...
    for risk in result.get("risk_flags") or []:
        ...
        if risk.startswith("не хватает ключевых сигналов роли:"):
            missing = ", ".join(
                SKILL_LABELS.get(skill.strip(), skill.strip())
                for skill in risk.split(":", 1)[1].split(",")
            )
            questions.append(f"Можете подробнее рассказать про опыт с {missing}?")
        elif risk == "желаемая роль отличается от рекомендованной":
            questions.append(
                "Какая роль вам все-таки ближе: выбранная изначально или та, к которой оказался ближе опыт?"
            )
            ...
            questions.append(
                "Были ли проекты с production-нагрузкой, реальными пользователями или бизнес-метриками?"
            )
        elif risk == "мало релевантного опыта":
            questions.append(
                "Есть ли стажировки, учебные или pet-проекты, которые стоит учесть?"
            )
...
class ValidateInterviewForm(FormValidationAction):
    ...
    async def extract_target_role(
        self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: DomainDict
    ) -> Dict[Text, Any]:
        if tracker.get_slot("requested_slot") != "target_role" and not entities(
            tracker, "target_role"
        ):
    ...
    async def extract_experience_years(
        self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: DomainDict
    ) -> Dict[Text, Any]:
        if tracker.get_slot("requested_slot") != "experience_years" and not entities(
            tracker, "experience_years"
        ):
            return {}
        text = lower_text(tracker)
        if is_out_of_scope_text(text):
            dispatcher.utter_message(
                text="Давайте вернемся к интервью. Сейчас важно понять ваш релевантный опыт."
            )
            return {"experience_years": None}
        if is_skip_like(text) or any(
            word in text for word in ["бариста", "студент", "учусь"]
        ):
    ...
    async def extract_skills(
        ...
        latest_intent = (tracker.latest_message.get("intent") or {}).get("name")
        if tracker.get_slot("requested_slot") != "skills" and latest_intent not in {
            "provide_skills",
            "provide_multiple_fields",
        }:
            return {}
        text = lower_text(tracker)
    ...
    async def extract_project_types(
        self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: DomainDict
    ) -> Dict[Text, Any]:
        if tracker.get_slot("requested_slot") != "project_types" and not entities(
            tracker, "project_type"
        ):
            return {}
        if is_skip_like(lower_text(tracker)):
            return {
                "project_types": ["not_specified"],
    ...
    async def extract_project_role(
        self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: DomainDict
    ) -> Dict[Text, Any]:
        if tracker.get_slot("requested_slot") != "project_role" and not entities(
            tracker, "project_role"
        ):
            return {}
        text = lower_text(tracker)
        if is_skip_like(text) or any(
            phrase in text for phrase in ["никакая роль", "роли не было", "нет роли"]
        ):
            return {"project_role": "unknown"}
        return extract_facts(tracker)

    async def extract_education_text(
        self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: DomainDict
    ) -> Dict[Text, Any]:
    ...
    async def extract_english_level(
        self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: DomainDict
    ) -> Dict[Text, Any]:
        if tracker.get_slot("requested_slot") != "english_level" and not entities(
            tracker, "english_level"
        ):
            return {}
        text = lower_text(tracker)
        if is_skip_like(text):
        ...
        english = normalize_english(
            text, (entities(tracker, "english_level") or [None])[0]
        )
        if english != "unknown":
            return {"english_level": english}
        return extract_facts(tracker)

    async def extract_work_format(
        self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: DomainDict
    ) -> Dict[Text, Any]:
        if tracker.get_slot("requested_slot") != "work_format" and not entities(
            tracker, "work_format"
        ):
            return {}
        if is_skip_like(lower_text(tracker)):
    ...
    async def extract_salary_expectation_min(
        self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: DomainDict
    ) -> Dict[Text, Any]:
        if tracker.get_slot(
            "requested_slot"
        ) != "salary_expectation_min" and not entities(tracker, "salary_amount"):
            return {}
        return extract_facts(tracker)
    ...
    async def extract_availability(
        ...
        if tracker.get_slot("requested_slot") != "availability" and not entities(
    ...
    def validate_experience_years(
        ...
        if slot_value is None:
            if not is_out_of_scope_text(lower_text(tracker)):
                dispatcher.utter_message(
                    text="Укажите опыт числом лет, например: 3 года. Если релевантного опыта нет, можно так и написать."
                )
            return {"experience_years": None}
    ...
    def validate_skills(
        ...
        if not slot_value:
            if not is_role_skill_question(lower_text(tracker)):
                dispatcher.utter_message(
                    text="Перечислите хотя бы несколько навыков или инструментов."
                )
            return {"skills": None}
    ...
    def validate_project_types(
        ...
        if not slot_value:
            dispatcher.utter_message(
                text="Расскажите хотя бы об одном проекте или напишите, что проектов пока не было."
            )
    ...
    def validate_project_role(
        ...
        if not slot_value or slot_value == "unknown":
            if "no_relevant_projects" in (tracker.get_slot("project_types") or []):
                return {"project_role": "unknown"}
            dispatcher.utter_message(
                text="Уточните вашу роль в проектах: управляли, анализировали, разрабатывали или лидировали?"
            )
            return {"project_role": None}
        return {"project_role": slot_value}

    def validate_education_text(
        self,
        slot_value: Any,
        dispatcher: CollectingDispatcher,
        tracker: Tracker,
    ...
    def validate_salary_expectation_min(
        self,
        slot_value: Any,
        dispatcher: CollectingDispatcher,
        tracker: Tracker,
        ...
        if slot_value is None:
            dispatcher.utter_message(
                text="Укажите зарплатные ожидания числом или диапазоном."
            )
...
class ActionRankCandidate(Action):
    ...
    def run(
        ...
        if result["tie_breaker_question"]:
            dispatcher.utter_message(
                text=f"Хочу уточнить, чтобы точнее сопоставить ваш опыт с вакансией. {result['tie_breaker_question']}"
            )
            return [
                SlotSet("role_ranking", result["ranking"]),
                SlotSet("risk_flags", result["risk_flags"]),
...
class ActionApplyTieBreaker(Action):
    ...
    def run(
        ...
        if not tracker.get_slot("tie_breaker_question"):
            dispatcher.utter_message(
                text="Сейчас нет активного уточняющего вопроса. Можем продолжить интервью."
            )
            return []
        slots = tracker.current_slot_values()
...
class ActionChangeAnswer(Action):
    ...
    def run(
        ...
        work_format = normalize_work_format(
            text, (entities(tracker, "work_format") or [None])[0]
        )
        availability = normalize_availability(
            text, (entities(tracker, "availability") or [None])[0]
        )
        ...
        if changed:
            ...
            dispatcher.utter_message(
                text="Не понял, что именно нужно изменить. Например: изменить зарплатные ожидания на 250к."
            )
        return events
...
class ActionRepeatQuestion(Action):
    ...
    def run(
        ...
        if question:
            ...
            dispatcher.utter_message(
                text="Сейчас нет активного вопроса. Можно начать интервью или продолжить отклик."
            )
        return []
...
class ActionShowNextStep(Action):
    ...
    def run(
        ...
        if tie_question:
            dispatcher.utter_message(
                text="Сначала ответьте на уточняющий вопрос: " + tie_question
            )
            return []
        if not report:
...
class ActionShowRoleDetails(Action):
    ...
    def run(
        ...
        if role == "unknown":
            role = infer_role(lower_text(tracker))
        dispatcher.utter_message(
            text=ROLE_DETAILS.get(
                role,
                "Уточните роль: Project Manager, Data Analyst, Data Engineer, Data Scientist или MLOps Engineer.",
            )
        )
        return []
